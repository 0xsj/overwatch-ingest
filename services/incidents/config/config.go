// services/incidents/config/config.go
package config
...
// Config defines the complete configuration for the Incidents service.
...
// Load loads the Incidents configuration from environment variables.
func Load(usePrefix bool) (Config, error) {
	...
	// Application config
	port, err := pkgconfig.LoadPortOptional(key("PORT"), 8082)
	if err != nil {
		return nil, err
	}
